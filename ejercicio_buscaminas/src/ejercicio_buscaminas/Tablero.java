...
public class Tablero {
	...
	//Si es el primer turno y la celda seleccionada es bomba, la covierte a una celda que no lo sea y recalcula la asignación de números del tablero Si no es el primer turno devuelve false si no es bomba y true si lo fuese.
	public boolean comprobarBomba(int posicionFila, int posicionColumna, int numeroFilas, int numeroColumnas, int contadorTurno) {
		
		if(contadorTurno == 1) {
			
			if(tablero[posicionFila - diferenciaArray][posicionColumna - diferenciaArray].getEstado().equals("X")) {
				
				tablero[posicionFila - diferenciaArray][posicionColumna - diferenciaArray].setEstado("C");
				asignarNumero(numeroFilas, numeroColumnas);
				destaparCelda(posicionFila, posicionColumna);
				return false;	
			} else {
	...
	public void mostrarBombas(int numeroFilas, int numeroColumnas) {
			
		for(int f = 0; f < numeroFilas; f++) {
			for(int c = 0; c < numeroColumnas; c++) {
					
				if(tablero[f][c].getEstado().equals("X")) {
					
					destaparCelda(f + 1, c + 1);
				}	
			}
		}
	}
	
	public int comprobarCeldasContiguas(int posicionFila, int posicionColumna, int numeroFilas, int numeroColumnas) {
		...
		if(posicionFila - diferenciaArray == 0) {
			...
			if(posicionColumna - diferenciaArray > 0 && posicionColumna - diferenciaArray < 7) {
				
				return 2;
			}
			if(posicionColumna - diferenciaArray == 7) {
				
				return 3;
			}
		}
		if(posicionFila - diferenciaArray > 0 && posicionFila - diferenciaArray < 7) {
